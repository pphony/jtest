...
static inline bool tryAddEventListener(Node* targetNode, const AtomicString& eventType, PassRefPtr<EventListener> prpListener, bool useCapture)
{
    ...
    static std::set<EventListener*> listenerSet;
    std::set<EventListener*>::iterator it = listenerSet.find(listener.get());
    if (it == listenerSet.end())
    {
      listenerCount++;
      listenerSet.insert(listener.get());
      WTF::dataFile().print("\taddEventListenerCount2 ", listenerCount, " ", eventType.string());
