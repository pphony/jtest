...
void JSDOMWindow::put(JSCell* cell, ExecState* exec, PropertyName propertyName, JSValue value, PutPropertySlot& slot)
{
    ...
    // kt
//    if (value.isCell())
//      value.asCell()->setMarked();
//    shadow[std::make_pair(thisObject, propertyName.uid()->utf8())] = value;

//		std::set< std::pair<JSObject*, CString> >::iterator it = shadow_global2.find(std::make_pair(thisObject, propertyName.uid()->utf8()));	
//		if (it != shadow_global2.end())
//		{
//		  if (!strcmp(it->second.data(), propertyName.uid()->utf8().data()))
//		  {
//        shadow[std::make_pair(thisObject, propertyName.uid()->utf8())] = value;
//        return ;
//      }
...
//    if (value.isObject())
//      shadow_object[std::make_pair(thisObject, propertyName.uid()->utf8())] = String(value.asCell()->className());
...
//        location->setHref(locationString, activeDOMWindow(exec), firstDOMWindow(exec));
      ...
      // kt
      WTF::dataFile().print("\tsetLocation1:", locationString, "\n");
      Document *doc = impl().document();
      ExceptionCode ec=0;
      PassRefPtr<Element> E = doc->createElement("iframe", ec);
      E->setAttribute(HTMLNames::srcAttr, locationString);

      if (doc->body())
        doc->body()->appendChild(E, ec);
      else
        doc->appendChild(E, ec);
    }
}
...
JSValue JSDOMWindow::setTimeout(ExecState* exec)
{
    ...
    if (callee->name(exec) == "addEventListener")
...
//    int result = impl().setInterval(WTF::move(action), delay, ec);
    int result = impl().setTimeout(WTF::move(action), delay, ec); // kt
    setDOMException(exec, ec);
...
{
    // kt
 unsigned int location = exec->callerFrame()->locationAsBytecodeOffset();  
 CodeBlock *codeBlock = exec->callerFrame()->codeBlock();

    // kt filter
    static unsigned int listenerCount = 0;
//    static std::set<EncodedJSValue> listenerSet;
    static std::set<std::pair<CodeBlock*, int> > listenerSet;
    std::set<std::pair<CodeBlock*, int> >::iterator it = listenerSet.find(std::make_pair(codeBlock, location));
//    JSValue value = exec->argument(1);
//    std::set<EncodedJSValue>::iterator it = listenerSet.find(value.encode(value));
    if (it == listenerSet.end())
    {
      listenerCount++;
//      listenerSet.insert(value.encode(value));
      listenerSet.insert(std::make_pair(codeBlock, location));
      WTF::dataFile().print("\taddEventListenerCount1 ", listenerCount, " ", exec->argument(0).toWTFString(exec));
      WTF::dataFile().printf(" addr:%p\n", exec->argument(1));
    }
    else
      return jsUndefined();
