...
namespace WebCore {
  ...
  // kt
static std::set<unsigned> urlFilter;
...
bool ContainerNode::insertBefore(PassRefPtr<Node> newChild, Node* refChild, ExceptionCode& ec)
{
    ...
    // kt filter out
    HTMLScriptElement *script = NULL;
    unsigned stringHash;
    if (newChild.get() != NULL && newChild.get()->nodeName() == "SCRIPT")
    {
      script = toHTMLScriptElement(newChild.get());
      stringHash = script->src().string().ascii().hash();

      std::set<unsigned>::iterator it = urlFilter.begin();
      for (; it != urlFilter.end(); it++)
      {
        if (*it == stringHash)
          return false;
      }
...
//    if (newChild.get() != NULL && newChild.get()->nodeName() == "SCRIPT")
    if (script != NULL )
    {
//      HTMLScriptElement *script = toHTMLScriptElement(newChild.get());
      WTF::dataFile().print("insertBefore(script) :", script->src().string(), "\n");
      urlFilter.insert(script->src().string().ascii().hash());
      if (!script->text().isEmpty())
        WTF::dataFile().print("\ttext: ", script->text(), "\n");
    }
    else if (newChild.get() != NULL && newChild.get()->nodeName() == "IFRAME")
    {
      HTMLIFrameElement *iframe = toHTMLIFrameElement(newChild.get());
      WTF::dataFile().print("insertBefore(iframe) :", iframe->location().string(), "\n");
    }
    else if (newChild.get() != NULL && newChild.get()->nodeName() == "IMG")
    {
      HTMLImageElement *img = toHTMLImageElement(newChild.get());
      WTF::dataFile().print("insertBefore(img) :", img->src().string(), "\n");
    }
    else if (newChild.get() != NULL)
    {
      WTF::dataFile().print("insertBefore(", newChild.get()->nodeName(),")\n");
        WTF::dataFile().print("\ttext: ", newChild.get()->textContent(), "\n");
    }
...
bool ContainerNode::removeChild(Node* oldChild, ExceptionCode& ec)
{
    ...
    // kt 
    WTF::dataFile().print("removeChild(", oldChild->nodeName(),")\n");
...
bool ContainerNode::appendChild(PassRefPtr<Node> newChild, ExceptionCode& ec)
{
    ...
    if (script != NULL)
    {
      WTF::dataFile().print("appendChild(script) :", script->src().string(), "\n");
      urlFilter.insert(script->src().string().ascii().hash());
      if (!script->text().isEmpty())
        WTF::dataFile().print("\ttext: ", script->text(), "\n");
    }
    ...
    {
      HTMLIFrameElement *iframe = toHTMLIFrameElement(newChild.get());
      WTF::dataFile().print("appendChild(iframe) :", iframe->location().string(), "\n");
    }
    else if (newChild.get() != NULL && newChild.get()->nodeName() == "IMG")
    {
      HTMLImageElement *img = toHTMLImageElement(newChild.get());
      WTF::dataFile().print("appendChild(img) :", img->src().string(), "\n");
    }
    else if (newChild.get() != NULL)
    {
      WTF::dataFile().print("appendChild(", newChild.get()->nodeName(),")\n");
        WTF::dataFile().print("\ttext: ", newChild.get()->textContent(), "\n");
    }
...
PassRefPtr<NodeList> ContainerNode::getElementsByTagName(const AtomicString& localName)
{
    ...
    {
      // kt
      PassRefPtr<NodeList> temp = ensureRareData().ensureNodeLists().addCacheWithAtomicName<HTMLTagNodeList>(*this, localName);
      
      if (!temp.get()->length())
      {
        ExceptionCode ec=0;
        PassRefPtr<Element> E = document().createElement(localName, ec);

        if (isElementNode())  // including HTMLHtmlElement, HTMLHeadElement, HTMLBodyElement
          insertBefore(E, firstChild(), ec);
        else if (isDocumentNode())
          document().insertBefore(E, document().firstChild(), ec);
        else 
          document().body()->insertBefore(E, document().body()->firstChild(), ec);
...
//        return ensureRareData().ensureNodeLists().addCacheWithAtomicName<HTMLTagNodeList>(*this, localName);
    ...
    PassRefPtr<NodeList> temp = ensureRareData().ensureNodeLists().addCacheWithAtomicName<TagNodeList>(*this, localName);
...
{
// kt
    PassRefPtr<NodeList> temp = ensureRareData().ensureNodeLists().addCacheWithAtomicName<NameNodeList>(*this, elementName);
    if (!temp.get()->length())
    {
      ExceptionCode ec=0;
      PassRefPtr<Element> E = document().createElement("div", ec);
      E->setAttribute(HTMLNames::nameAttr, elementName);
...
{
// kt
    PassRefPtr<NodeList> temp = ensureRareData().ensureNodeLists().addCacheWithName<ClassNodeList>(*this, classNames);
    if (!temp.get()->length())
    {
      ExceptionCode ec=0;
      PassRefPtr<Element> E = document().createElement("div", ec);
      E->setAttribute(HTMLNames::classAttr, classNames);
